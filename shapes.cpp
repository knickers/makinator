...
void DrawSolidCylinder(double length, double xR, double yR, double slices, bool hollow)
{
	...
	for(double i=0; i<=2*PI; i+=inc) {
		...
		glBegin(GL_QUADS);
		glNormal3d(x,y,0);
		glVertex3d(xR*x, yR*y, 0);
		glVertex3d(xR*x, yR*y, length);
		// Next x and y
		x = cos(i+inc);
		y = sin(i+inc);
		// Next Walls
...
bool ExtrudePolygon(char file[], double height, bool smooth) {
	ifstream fin(file);
	...
	// Extrude the polygon
	ExtrudePolygon(outline, height, smooth);
	return 1;
}
bool ExtrudePolygon(vector<point2> outline, double height, bool smooth) {
	glBegin(GL_QUADS);
	for (unsigned i=0; i<outline.size()-1; i++) {
		point3 a = {outline[ i ].x, outline[ i ].y, 0};
		point3 b = {outline[ i ].x, outline[ i ].y, height};
		point3 c = {outline[i+1].x, outline[i+1].y, height};
		point3 d = {outline[i+1].x, outline[i+1].y, 0};
		point3 n = FindTriangleNormal(a, d, b);
...
// Draws a 3d triangle with the specified coordinates and height
void DrawExtrudeTriangle(double x1,double y1,double x2,double y2,double x3,double y3,double height) {
	glPushMatrix();
		glTranslated(0,0,height/2);
			glNormal3d(0,0,1); // Upper surface
			DrawTriangle(x1,y1, x2,y2, x3,y3);
			glTranslated(0,0,-height);
				glNormal3d(0,0,-1); // Lower surface
				DrawTriangle(x1,y1, x2,y2, x3,y3);
	glPopMatrix();
	double a = atan2(x1-x2,y1-y2);
	glNormal3d(cos(a), sin(a), 0);
	DrawRectangle3d((point3){x1,y1,-height/2}, (point3){x2,y2,height/2});
	a = atan2(x3-x2,y3-y2);
	glNormal3d(cos(a-PI/2), sin(a-PI/2), 0);
	DrawRectangle3d((point3){x2,y2,-height/2}, (point3){x3,y3,height/2});
	a = atan2(x1-x3,y1-y3);
	glNormal3d(cos(a), sin(a), 0);
	DrawRectangle3d((point3){x3,y3,-height/2}, (point3){x1,y1,height/2});
}
...
void DrawTable(double x, double y, double z) {
	...
	// Legs
	glPushMatrix();
		glTranslated(x/2 - ax/2, -y/2 + ay/2, 0);
			DrawSolidBox(ax, ay, z-az); // q4
			glTranslated(-x+ax, 0, 0);
				DrawSolidBox(ax, ay, z-az); // q3
				glTranslated(0, y-ay, 0);
			DrawSolidBox(ax, ay, z-az); // q2
			glTranslated(x-ax, 0, 0);
